...
function NestCamAccessory(conn, log, device, structure, platform) {
  ...
  const switchSvc = this.addService(Service.Switch);
  this.bindCharacteristic(switchSvc, Characteristic.On, "Streaming",
    getStreamingState.bind(this), setStreamingState.bind(this), null);
...
const getStreamingState = function() {
  return this.device.is_streaming;
};

const setStreamingState = function(targetStreamingState, callback) {
  return this.updateDevicePropertyAsync("is_streaming", targetStreamingState == 1, "target streaming state")
    .asCallback(callback);
};
